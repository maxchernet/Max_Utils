...
for srr_n in range(0, 24):

    fname_srr = dir_in_ref + f_in_ref % srr_n
    if os.path.isfile(fname_srr) is False:
        print('FIle %s does not exist' % fname_srr)
        continue
    gds_ref = gdal.Open(fname_srr)
    ...
    gds_out = driver.Create(dir_out + 'SRR_%d_n%d_opt_opt_%s.tif' %
                           (srr_n, r_val_arr0.argmax(), band), ssize, ssize, 1, type_in)
    # gds_out.GetRasterBand(1).WriteArray(img[j:j+ssize, i:i+ssize])
